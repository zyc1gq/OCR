...
def train(train_dir=None, val_dir=None, mode='train'):
    model = cnn_ocr.LSTMOCR(mode)
    model.build_graph()

    print('loading train data, please wait---------------------')
    train_feeder = util_p.DataIterator(img_dir=train_dir,lab_dir= FLAGS.tra_lab,rpath="image/",t_num=8200000)
    print('get image: ', train_feeder.size)

    print('loading validation data, please wait---------------------')
    val_feeder = util_p.DataIterator(img_dir=val_dir,lab_dir=FLAGS.val_lab,rpath="image_test/",t_num=8200000)
